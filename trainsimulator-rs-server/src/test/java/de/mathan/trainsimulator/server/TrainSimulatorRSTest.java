...
public class TrainSimulatorRSTest extends JerseyTest {
    ...
    resourceConfig.register(
        new AbstractBinder() {
          ...
          protected void configure() {
            bind(TrainSimulatorRSTest.this.mock).to(NativeLibrary.class);
          }
        });
  ...
  @Before
  public void prepare() {
    Mockito.when(this.mock.GetLocoName()).thenReturn(TEST_LOCO);
    Mockito.when(this.mock.GetRailSimCombinedThrottleBrake()).thenReturn(Boolean.TRUE);
  }

  @Test
  public void getLocomotive() {
    Mockito.when(this.mock.GetControllerList()).thenReturn(TEST_CONTROL_LIST);
    Response response =
        target("/trainsimulator/locomotive").request(MediaType.APPLICATION_JSON_TYPE).get();
  ...
  public void getController() {
    Mockito.when(this.mock.GetControllerList()).thenReturn(TEST_CONTROL_LIST);
    Mockito.when(this.mock.GetControllerValue(TEST_CONTROL_1_ID, TYPE_CURRENT))
        .thenReturn(TEST_VALUE_CURRENT);
    Mockito.when(this.mock.GetControllerValue(TEST_CONTROL_1_ID, TYPE_MINIMUM))
        .thenReturn(TEST_VALUE_MINIMUM);
    Mockito.when(this.mock.GetControllerValue(TEST_CONTROL_1_ID, TYPE_MAXIMUM))
        .thenReturn(TEST_VALUE_MAXIMUM);
    Response response =
        target("/trainsimulator/locomotive").request(MediaType.APPLICATION_JSON_TYPE).get();
    Assert.assertEquals(200, response.getStatus());
    response =
        target("/trainsimulator/control/" + Control.SifaLight.getValue())
  ...
  public void defaultControls() {
    Mockito.when(this.mock.GetControllerList())
        .thenReturn(controls(TEST_DEFAULT_CONTROL_PZB_500, TEST_DEFAULT_CONTROL_VIGIL_LIGHT));
    Response response =
        target("/trainsimulator/locomotive").request(MediaType.APPLICATION_JSON_TYPE).get();
    Assert.assertEquals(200, response.getStatus());
  ...
  public void mappedControls() {
    Mockito.when(this.mock.GetControllerList())
        .thenReturn(controls(TEST_MAPPED_CONTROL_PZB_500, TEST_MAPPED_CONTROL_VIGIL_LIGHT));
    Response response =
        target("/trainsimulator/locomotive").request(MediaType.APPLICATION_JSON_TYPE).get();
    Assert.assertEquals(200, response.getStatus());
    Locomotive loco = response.readEntity(Locomotive.class);
  ...
  public void virtualControls() {
    ...
    Mockito.when(this.mock.GetControllerValue(TEST_CONTROL_1_ID, TYPE_CURRENT))
        .thenReturn(
            TEST_VIRTUAL_CONTROL_VALUE_PZB_55,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_55,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_55,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_70,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_70,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_70,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_85,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_85,
            TEST_VIRTUAL_CONTROL_VALUE_PZB_85);
    Response response =
        target("/trainsimulator/locomotive").request(MediaType.APPLICATION_JSON_TYPE).get();
  ...
  private void assertControlValue(Control control, Float expectedValue) {
    Response response =
        target("/trainsimulator/control/" + control.getValue())
            .request(MediaType.APPLICATION_JSON)
            .get();
    Assert.assertEquals(200, response.getStatus());
    ControlValue value = response.readEntity(ControlValue.class);
